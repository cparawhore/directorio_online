...
$app->get('/eventos', function () use ($app){
    try{
		...
		$dbh = $connection->prepare("SELECT *,datediff(fec_evento, curdate()) as diferencia FROM eventos WHERE datediff(fec_evento, curdate())");
		$dbh->execute();
...
$app->get('/page/:id', function ($id) use ($app){
    try{
		...
		$numero = $count->fetch();
		$connection = null;
		if(!is_string($id)) $app->render('inmueble.php', array('inmuebles'=> $inmuebles , 'var'=> $numero[0], 'num_page'=> $id ));
